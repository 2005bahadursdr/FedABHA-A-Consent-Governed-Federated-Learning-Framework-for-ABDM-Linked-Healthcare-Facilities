  ...
  return (
    <div className="flex min-h-screen lg:h-screen bg-dark-900 text-slate-100 font-sans overflow-x-hidden w-full max-w-full">
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-screen lg:min-h-0 overflow-y-auto lg:overflow-hidden">
        <header className="sticky top-0 z-30 h-16 bg-dark-800/95 backdrop-blur-md border-b border-slate-700 flex items-center px-4 sm:px-6 lg:px-8 shadow-sm justify-between shrink-0">
          <div className="flex items-center space-x-3">
            <button
              onClick={() => setMobileOpen(true)}
              className="lg:hidden text-slate-300 hover:text-slate-100 p-1.5 bg-dark-700 border border-slate-600 rounded-lg transition-colors"
              aria-label="Open Mobile Menu"
            >
              <Menu className="w-5 h-5" />
            </button>
            <div className="text-base sm:text-lg font-semibold text-slate-200 truncate">
          ...
          <div className="flex items-center space-x-3 shrink-0">
            ...
            {user && (
              <button
                ...
                className={cn(
                  "flex items-center space-x-2 px-3 py-1.5 rounded-lg border text-xs font-semibold transition shadow-sm",
                  getRoleBadgeStyle()
                )}
                title="Click to Switch RBAC Role (Super Admin / Hospital Admin / Auditor)"
              >
                <UserCheck className="w-3.5 h-3.5" />
                <span className="hidden md:inline">{user.name} ({user.role_title})</span>
                <span className="md:hidden">{user.role_title}</span>
            ...
            <button
              onClick={toggleTheme}
              className="flex items-center justify-center p-2 rounded-lg bg-dark-700 hover:bg-slate-700 text-amber-400 border border-slate-600 transition-colors shadow-sm"
            ...
            <span className="flex items-center text-xs sm:text-sm text-green-400 font-medium">
              <span className="w-2 h-2 rounded-full bg-green-500 mr-2 animate-pulse shrink-0"></span>
